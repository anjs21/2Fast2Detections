...
class UncertaintyLoss(nn.Module):
    ...
    def get_weights(self):
        # Return effective precision weights (exp(-log_var)), matching the loss formula.
        return torch.exp(-self.log_var[0]).item(), torch.exp(-self.log_var[1]).item()
...
def train_multitask_model(model, train_loader, val_loader, config, loss_weighter=None, tag="multitask"):
    ...
    if isinstance(loss_weighter, nn.Module):
        ...
        trainable_params += [p for p in loss_weighter.parameters() if p.requires_grad]
    ...
    for epoch in range(total_epochs):
        ...
        if isinstance(loss_weighter, GradNormBalancer) or isinstance(loss_weighter, UncertaintyLoss):
