...
def run_ablation_study(train_loader, val_loader):
    ...
    """
    weight_configs = [
        ("static",      1.0,  0.0,  "binary_only (1 - 0)"),
        ("static",      0.75, 0.25, "binary_dominant (0.75 - 0.25)"),
        ("static",      0.5,  0.5,  "equal (0.5 - 0.5)"),
        ("static",      0.25, 0.75, "transform_dominant (0.25 - 0.75)"),
        ("static",      0.0,  1.0,  "transform_only (0 - 1)"),
    ...
    for method, w1, w2, label in weight_configs:
        print(f"\n--- Config: method={method}, w1={w1}, w2={w2} ({label}) ---")
        ...
        model, logger = train_multitask_model(
            model, train_loader, val_loader, CONFIG, loss_weighter=loss_weighter, tag=f"ablation_{label}"
        )
